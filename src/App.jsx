...
projectsPersonal: [
{ id: 1, name: “Ãcharpe hiver”, rang: 42, total: 80, colorIdx: 0, image: null, type: “tricot”, laine: “”, outil: “”, notes: “”, parties: [] },
{ id: 2, name: “Tuque NoÃ«l”, rang: 15, total: 30, colorIdx: 1, image: null, type: “crochet”, laine: “”, outil: “”, notes: “”, parties: [] },
{ id: 3, name: “Mitaines bÃ©bÃ©”, rang: 8, total: 20, colorIdx: 2, image: null, type: “tricot”, laine: “”, outil: “”, notes: “”, parties: [] },
],
projectsPro: [],
patrons: [],
...
console.warn(”[KALEIDO] saveToDatabase ignorÃ©: base invalide.”);
return false;
}
...
<p style={{ color: "#999", fontSize: 13, margin: "0 0 20px" }}>Cette action est irrÃ©versible.</p>
...
ctx.clip();
// baseScale : l'image doit COUVRIR le cercle entiÃ¨rement Ã  scale=1
const baseScale = Math.max(CROP_SIZE / img.width, CROP_SIZE / img.height);
const finalScale = baseScale * scale;
// Taille affichÃ©e dans le canvas de sortie
...
{/* Overlay sombre autour du cercle â ne couvre PAS les boutons */}
...
<p style={{ color: "#fff", fontSize: 14, margin: 0, textAlign: "center" }}>DÃ©place et zoome pour recadrer</p>
</div>
{/* Zone de recadrage */}
...
{/* Boutons â toujours visibles, z-index Ã©levÃ© */}
...
<button onClick={handleConfirm} style={{ padding: "14px 30px", borderRadius: 14, border: "none", background: "linear-gradient(135deg, #7C3AED, #EC4899)", color: "#fff", fontSize: 16, fontWeight: 700, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>Confirmer â</button>
</div>
...
<button onClick={e => handleActionClick(e, () => onDelete(rang.id))} style={{ background: "#DC2626", border: "none", borderRadius: 6, width: 32, height: 32, display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 12, cursor: "pointer" }}>â</button>
</div>
</div>
);
}
...
<div style={{ color: "#666", fontSize: 11, fontStyle: "italic" }}>Swipe â pour actions â¢ Clic pour modifier</div>
</div>
)}
</div>
<div style={{ display: "flex", flexDirection: "column", gap: 4, flexShrink: 0 }}>
...
{/* Nom centrÃ© + rangs centrÃ© â deux colonnes */}
...
<button onClick={() => setIsCollapsed(!isCollapsed)} style={{ background: "none", border: "none", color: color.light, fontSize: 14, cursor: "pointer", padding: 4, flexShrink: 0 }}>{isCollapsed ? "â¼" : "â²"}</button>
<div style={{ display: "flex", gap: 4, flexShrink: 0 }}>
{[
{ icon: "â", dis: isFirst, fn: () => onMoveUp(partie.id), bg: isFirst ? "#333" : color.bg },
{ icon: "â", dis: isLast, fn: () => onMoveDown(partie.id), bg: isLast ? "#333" : color.bg },
{ icon: "â§", dis: false, fn: () => onDuplicate(partie.id), bg: color.bg },
{ icon: "â", dis: false, fn: () => onDelete(partie.id), bg: "#DC2626" },
].map((b, i) => (
...
Swipe vers la gauche sur les rangs pour dupliquer â§ et supprimer â
</div>
...
// âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
// COMPTEUR WIDGET (indÃ©pendant, swipe corrigÃ©)
// âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
// Swipe handlers â sÃ©parÃ©s pour Ã©viter les conflits
...
style={{ background: counter.syncWithGlobal ? "#333" : col.bg, border: "none", borderRadius: "50%", width: 32, height: 32, color: "#fff", fontSize: 16, cursor: counter.syncWithGlobal ? "not-allowed" : "pointer", fontWeight: "bold", display: "flex", alignItems: "center", justifyContent: "center" }}>â</button>
...
{counter.syncWithGlobal && <div style={{ color: "#666", fontSize: 10, marginTop: 6, fontStyle: "italic" }}>Sync â¢ Reset aprÃ¨s {counter.maxRepeats || 4}</div>}
</div>
{/* Actions swipe â stopPropagation sur chaque bouton */}
...
{isSwipedOpen && <div style={{ position: "absolute", bottom: 6, right: 8, color: col.light, fontSize: 8, opacity: 0.7 }}>â</div>}
</div>
);
}
// âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
// COMPTEUR DE RANGS (composant indÃ©pendant â corrige le bug reset)
// âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
{swiped && <div style={{ position: "absolute", bottom: 14, right: 4, color: currentPartieColor.light, fontSize: 8, opacity: 0.6 }}>â</div>}
</div>
);
}
...
// On est sur une note â chercher le dernier rang normal avant dans la partie
...
<button onClick={onNavigateHub} style={{ background: "#1E1E32", border: "none", borderRadius: 10, width: 36, height: 36, display: "flex", alignItems: "center", justifyContent: "center", color: "#A78BFA", fontSize: 16, cursor: "pointer" }}>â</button>
...
<div style={{ fontSize: 16, marginBottom: 8, color: "#F1F0EE" }}>Aucun patron crÃ©Ã©</div>
<div style={{ fontSize: 13, marginBottom: 24 }}>CrÃ©e d'abord tes parties et rangs dans l'Ã©diteur de patron</div>
<button onClick={() => onNavigateEditor(project)} style={{ background: "linear-gradient(135deg, #7C3AED, #EC4899)", border: "none", borderRadius: 12, padding: "12px 24px", color: "#fff", fontSize: 14, cursor: "pointer", fontWeight: 600 }}>Ouvrir l'Ã©diteur</button>
</div>
</div>
);
...
<button onClick={() => { onSaveProgress(allRangs.slice(0, currentIndex + 1).filter(r => !r.isNote).length, totalRangsForCount, elapsedTime); onNavigateHub(); }} style={{ background: "#1E1E32", border: "none", borderRadius: 10, width: 36, height: 36, display: "flex", alignItems: "center", justifyContent: "center", color: "#A78BFA", fontSize: 16, cursor: "pointer" }}>â</button>
<div style={{ flex: 1 }}>
<h1 style={{ color: "#F1F0EE", margin: 0, fontSize: 16, fontWeight: 700, fontFamily: "'Syne', sans-serif" }}>{patron.nom}</h1>
<div style={{ color: "#A78BFA", fontSize: 11, fontFamily: "monospace", marginTop: 2 }}>{patron.technique}{patron.outil ? ` â¢ ${patron.outil}` : ""}</div>
</div>
...
{/* Prochain rang - seulement si on reste dans la mÃªme partie */}
...
{/* Modale passage Ã  la partie suivante */}
{showNextPartieModal && (() => {
...
<div style={{ color: currentPartieColor.light, fontSize: 13, fontFamily: "monospace", textTransform: "uppercase", letterSpacing: 1, marginBottom: 8 }}>Partie terminÃ©e !</div>
<h2 style={{ color: "#F1F0EE", fontSize: 20, fontFamily: "'Syne', sans-serif", margin: "0 0 8px" }}>{currentPartie?.nom}</h2>
<p style={{ color: "#6B6A7A", fontSize: 14, margin: "0 0 24px" }}>Tu veux passer Ã  la partie suivante ?</p>
...
{/* Modale retour Ã  la partie prÃ©cÃ©dente */}
{showPrevPartieModal && (() => {
...
<div style={{ color: currentPartieColor.light, fontSize: 13, fontFamily: "monospace", textTransform: "uppercase", letterSpacing: 1, marginBottom: 8 }}>Revenir en arriÃ¨re ?</div>
<h2 style={{ color: "#F1F0EE", fontSize: 20, fontFamily: "'Syne', sans-serif", margin: "0 0 8px" }}>{currentPartie?.nom}</h2>
<p style={{ color: "#6B6A7A", fontSize: 14, margin: "0 0 24px" }}>Tu veux retourner Ã  la partie prÃ©cÃ©dente ?</p>
...
<div style={{ color: prevColor.light, fontSize: 12, fontFamily: "monospace", textTransform: "uppercase", letterSpacing: 1, marginBottom: 4 }}>Partie prÃ©cÃ©dente</div>
...
<input value={name} onChange={e => setName(e.target.value)} placeholder="Ex: Tuque NoÃ«l, Ãcharpe hiver..."
...
{pdfData && <div style={{ color: "#6B6A7A", fontSize: 11, marginTop: 2 }}>PDF chargÃ© â</div>}
</div>
...
<button onClick={() => removePartie(p.id)} style={{ width: 28, height: 28, borderRadius: 6, background: "#DC262633", border: "none", color: "#F87171", fontSize: 14, cursor: "pointer", flexShrink: 0 }}>â</button>
</div>
))}
...
// âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
// PDF ZOOM ZONE â pinch-zoom maison, indÃ©pendant du header
// âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
// Ajuster l'offset pour zoomer centrÃ© sur le point de pinch
...
<div style={{ fontSize: 48 }}>
            ...
            <div style={{ fontSize: 12, color: "#6B6A7A", textAlign: "center", padding: "0 40px" }}>Le fichier n'a pas pu Ãªtre chargÃ©</div>
          </div>
        ) : (
          pages.map((src, i) => (
    ...
    <div
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, flex: 1, transform: swiped ? "translateX(-100px)" : "translateX(0)", transition: "transform 0.3s ease" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 2, width: 80, flexShrink: 0 }}>
          ...
          <div style={{ position: "relative", width: 64, height: 64 }}>
            ...
            <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
              ...
              <span style={{ color: color.light, fontSize: 11, fontFamily: "monospace", marginTop: 1 }}>/{total > 0 ? total : "â"}</span>
            </div>
          </div>
        ...
        <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", justifyContent: "flex-start", marginTop: -4 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, transform: "translateX(-16px)" }}>
            <button onClick={decrementRang} style={{ width: 42, height: 42, borderRadius: "50%", background: `${color.bg}33`, border: `1.5px solid ${color.light}44`, color: color.light, fontSize: 22, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>â</button>
      ...
      <div style={{ position: "absolute", top: "50%", right: swiped ? 10 : -110, transform: "translateY(-50%)", transition: "right 0.3s ease", zIndex: 10, display: "flex", flexDirection: "column", gap: 6 }}>
        ...
        <button onClick={e => { e.stopPropagation(); resetRang(); setSwiped(false); }}
          style={{ background: "#DC2626", border: "none", borderRadius: 8, padding: "8px 12px", color: "#fff", fontSize: 11, fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap" }}>âº Reset</button>
      </div>
      {swiped && <div style={{ position: "absolute", bottom: 4, right: 8, color: color.light, fontSize: 8, opacity: 0.6 }}>â</div>}
    </div>
  );
}
function PdfViewerView({ project, onNavigateHub, onSaveProgress }) {
  ...
  const [loadErrorMessage, setLoadErrorMessage] = useState("Le fichier n'a pas pu Ãªtre chargÃ©");
  // Rangs
  ...
  const toggleTimer = () => { if (isTimerRunning) setIsTimerRunning(false); else { setStartTime(Date.now() - elapsedTime); setIsTimerRunning(true); } };
  const resetTimer = () => { setStartTime(Date.now()); setElapsedTime(0); setIsTimerRunning(true); };
  // IncrÃ©menter rang et changer de partie si nÃ©cessaire
  const incrementRang = () => {
  ...
  const decrementRang = () => {
    if (rang <= 0) return;
    const newRang = rang - 1;
    // DÃ©tecter si on revient Ã  la partie prÃ©cÃ©dente
    if (hasParties && currentPartieIdx > 0) {
      let offset = 0;
      for (let i = 0; i < currentPartieIdx; i++) offset += pdfParties[i].totalRangs;
      if (newRang < offset) {
        setShowPrevPartieModal(true);
        return; // Ne pas dÃ©crÃ©menter encore, attendre confirmation
      }
    }
    setRang(newRang);
  };
  ...
  // Chargement PDF â rendu page par page progressif
  useEffect(() => {
    let cancelled = false;
    setPages([]);
    setLoading(true);
    setLoadError(false);
    setLoadErrorMessage("Le fichier n'a pas pu Ãªtre chargÃ©");
    if (!project?.pdfId) {
      setLoadErrorMessage("Aucun PDF n'est associÃ© Ã  ce projet.");
      setLoadError(true);
      setLoading(false);
      return;
    }
    loadPdf(project.pdfId).then(async (data) => {
      ...
      try {
        // Charger pdf.js si pas encore chargÃ©
        if (!window.pdfjsLib) {
          await new Promise((resolve, reject) => {
        ...
        // DÃ©coder le base64 en chunks pour Ã©viter le crash mÃ©moire sur gros PDFs
        ...
        if (cancelled) return;
        setLoading(false); // Afficher le conteneur dÃ¨s que le PDF est parsÃ©
        // Scale adaptÃ© selon le nombre de pages â moins de mÃ©moire pour les gros PDFs
        ...
        for (let i = 1; i <= pdf.numPages; i++) {
          ...
          try {
            ...
            const imgData = canvas.toDataURL('image/jpeg', 0.85); // JPEG compressÃ© = moins de mÃ©moire
            // LibÃ©rer le canvas immÃ©diatement
            canvas.width = 0; canvas.height = 0;
            if (!cancelled) {
              setPages(prev => [...prev, imgData]);
            }
          } catch (pageErr) {
            console.warn(`Page ${i} Ã©chouÃ©e:`, pageErr);
            // Continuer avec les autres pages mÃªme si une Ã©choue
          }
        }
      } catch(e) {
        console.error('PDF error:', e);
        if (!cancelled) {
          setLoadErrorMessage("Le rendu du PDF a Ã©chouÃ©.");
          setLoadError(true);
          setLoading(false);
        }
      }
      ...
      if (!cancelled) {
        setLoadErrorMessage("Le chargement du PDF a Ã©chouÃ©.");
        setLoadError(true);
        setLoading(false);
      }
    });
  ...
  return (
    <div style={{ background: "#0D0D1A", height: "100vh", fontFamily: "'DM Sans', sans-serif", maxWidth: 430, margin: "0 auto", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      `}</style>
      {/* ââ BARRE FIXE ââââââââââââââââââââââââââââââââââââââââââ */}
      <div style={{ flexShrink: 0, background: "linear-gradient(180deg, #1A0A2E 0%, #0D0D1A 100%)", zIndex: 10, padding: "48px 16px 12px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
          <button onClick={handleBack} style={{ background: "#1E1E32", border: "none", borderRadius: 10, width: 34, height: 34, display: "flex", alignItems: "center", justifyContent: "center", color: color.light, fontSize: 15, cursor: "pointer", flexShrink: 0 }}>â</button>
      ...
      {/* ââ COMPTEURS SECONDAIRES ââââââââââââââââââââââââââââââ */}
      {counters.length > 0 && (
      ...
      {/* ââ ZONE PDF âââââââââââââââââââââââââââââââââââââââââââââ */}
        ...
        return (
          <div style={{ position: "fixed", inset: 0, zIndex: 200, background: "rgba(0,0,0,0.85)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
            <div style={{ background: "#1A1A2E", borderRadius: 24, padding: 28, width: "100%", maxWidth: 360, textAlign: "center", border: `1px solid ${color.light}33` }}>
              ...
              <div style={{ color: color.light, fontSize: 13, fontFamily: "monospace", textTransform: "uppercase", letterSpacing: 1, marginBottom: 8 }}>Partie terminÃ©e !</div>
              <h2 style={{ color: "#F1F0EE", fontSize: 20, fontFamily: "'Syne', sans-serif", margin: "0 0 8px" }}>{currentPartie?.nom}</h2>
              <p style={{ color: "#6B6A7A", fontSize: 14, margin: "0 0 24px" }}>Tu veux passer Ã  la partie suivante ?</p>
              {nextPartie && (
                <div style={{ background: `${nextColor.bg}22`, border: `1px solid ${nextColor.light}44`, borderRadius: 14, padding: "12px 16px", marginBottom: 24 }}>
              ...
              <div style={{ display: "flex", gap: 12 }}>
                ...
                <button onClick={() => {
                  // Calculer l'offset de la prochaine partie et mettre rang Ã  son premier rang
                  let offset = 0;
                  for (let i = 0; i <= currentPartieIdx; i++) offset += pdfParties[i].totalRangs;
                  setRang(offset + 1);
        ...
        return (
          <div style={{ position: "fixed", inset: 0, zIndex: 200, background: "rgba(0,0,0,0.85)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
            <div style={{ background: "#1A1A2E", borderRadius: 24, padding: 28, width: "100%", maxWidth: 360, textAlign: "center", border: `1px solid ${color.light}33` }}>
              ...
              <div style={{ color: color.light, fontSize: 13, fontFamily: "monospace", textTransform: "uppercase", letterSpacing: 1, marginBottom: 8 }}>Revenir en arriÃ¨re ?</div>
              <h2 style={{ color: "#F1F0EE", fontSize: 20, fontFamily: "'Syne', sans-serif", margin: "0 0 8px" }}>{currentPartie?.nom}</h2>
              <p style={{ color: "#6B6A7A", fontSize: 14, margin: "0 0 24px" }}>Tu veux retourner Ã  la partie prÃ©cÃ©dente ?</p>
              {prevPartie && (
                <div style={{ background: `${prevColor.bg}22`, border: `1px solid ${prevColor.light}44`, borderRadius: 14, padding: "12px 16px", marginBottom: 24 }}>
              ...
              <div style={{ display: "flex", gap: 12 }}>
                ...
                <button onClick={() => {
                  // Calculer le dernier rang de la partie prÃ©cÃ©dente
                  let offset = 0;
      ...
      {/* Modale projet terminÃ© */}
      {showFinModal && (
        <div style={{ position: "fixed", inset: 0, zIndex: 200, background: "rgba(0,0,0,0.85)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
          <div style={{ background: "#1A1A2E", borderRadius: 24, padding: 28, width: "100%", maxWidth: 360, textAlign: "center", border: `1px solid ${color.light}33` }}>
            <div style={{ fontSize: 52, marginBottom: 12 }}>
</div>
            <div style={{ color: color.light, fontSize: 13, fontFamily: "monospace", textTransform: "uppercase", letterSpacing: 1, marginBottom: 8 }}>Projet terminÃ© !</div>
            <h2 style={{ color: "#F1F0EE", fontSize: 20, fontFamily: "'Syne', sans-serif", margin: "0 0 8px" }}>{project?.name}</h2>
            <p style={{ color: "#6B6A7A", fontSize: 14, margin: "0 0 24px" }}>FÃ©licitations, tu as complÃ©tÃ© tous les rangs ! 
</p>
    ...
    <div style={{ background: "#0D0D1A", minHeight: "100vh", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{ padding: "52px 20px 16px", background: "linear-gradient(180deg, #1A0A2E 0%, #0D0D1A 100%)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
          <button onClick={onNavigateHub} style={{ background: "#1E1E32", border: "none", borderRadius: 10, width: 36, height: 36, display: "flex", alignItems: "center", justifyContent: "center", color: "#A78BFA", fontSize: 16, cursor: "pointer", flexShrink: 0 }}>â</button>
          <span style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: 20, background: "linear-gradient(135deg, #A78BFA, #F472B6)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}><span style={{ display: "inline-flex", alignItems: "center", gap: 8 }}><Icon name="library" size={26} color="#A78BFA" />BibliothÃ¨que</span></span>
          <div style={{ flex: 1 }} />
      ...
      <div style={{ padding: "12px 6px 100px" }}>
        {filtered.length === 0 ? (
          <div style={{ textAlign: "center", color: "#6B6A7A", padding: "60px 20px" }}>
            ...
            <div style={{ fontSize: 13 }}>CrÃ©e ou importe un patron avec le bouton +</div>
          </div>
        ) : (
          ...
          <div onClick={e => e.stopPropagation()} style={{ background: "#1A1A2E", borderRadius: "24px 24px 0 0", padding: "24px 20px 40px", width: "100%", maxWidth: 430 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <button onClick={() => { setShowNewMenu(false); onNewCustomPatron(); }} style={{ display: "flex", alignItems: "center", gap: 16, padding: "18px 20px", borderRadius: 16, background: "linear-gradient(135deg, #7C3AED22, #DB277722)", border: "1px solid #7C3AED44", cursor: "pointer", textAlign: "left" }}>
                ...
                <div>
                  <div style={{ color: "#F1F0EE", fontSize: 16, fontWeight: 700, fontFamily: "'DM Sans', sans-serif", marginBottom: 4 }}>CrÃ©er un patron</div>
              ...
              <button onClick={() => { setShowNewMenu(false); onNewPdfPatron(); }} style={{ display: "flex", alignItems: "center", gap: 16, padding: "18px 20px", borderRadius: 16, background: "linear-gradient(135deg, #0891B222, #22D3EE22)", border: "1px solid #0891B244", cursor: "pointer", textAlign: "left" }}>
                ...
                <div>
                  ...
                  <div style={{ color: "#6B6A7A", fontSize: 13, fontFamily: "'DM Sans', sans-serif" }}>TÃ©lÃ©charge un PDF et donne un nom</div>
                </div>
              </button>
            </div>
          </div>
...
// âââ PATRONS CRUD âââââââââââââââââââââââââââââââââââââââââââââ
...
if (!project) {
console.warn(”[KALEIDO] navigateToPatronEditor ignorÃ©: projet manquant.”);
return;
}
setViewTransition(‘slide-in-right’);
setTimeout(() => setViewTransition(‘none’), 350);
...
if (!project) {
console.warn(”[KALEIDO] navigateToRowCounter ignorÃ©: projet manquant.”);
return;
}
setViewTransition(‘slide-in-right’);
setTimeout(() => setViewTransition(‘none’), 350);
...
if (!project) {
console.warn(”[KALEIDO] navigateToPdfViewer ignorÃ©: projet manquant.”);
return;
}
if (!project.pdfId) {
...
// âââ VUE HUB ââââââââââââââââââââââââââââââââââââââââââââââ
const HubView = () => (

<div style={{ background: "#0D0D1A", minHeight: "100vh", fontFamily: "'DM Sans', sans-serif" }}>
<style>{`@import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600;700&family=Syne:wght@700;800&display=swap'); @keyframes fadeIn { from { opacity:0; transform:translateY(10px); } to { opacity:1; transform:translateY(0); } } ::-webkit-scrollbar { width: 0; } * { -webkit-tap-highlight-color: transparent; } input, textarea, select { font-size: 16px !important; }`}</style>
...
<div style={{ display: "flex", gap: 8 }}>
<button onClick={navigateToLibrary} aria-label="Ouvrir la bibliothÃ¨que de patrons" style={{ background: "#1E1E32", border: "none", borderRadius: 10, width: 40, height: 40, display: "flex", alignItems: "center", justifyContent: "center", color: "#A78BFA", cursor: "pointer" }}><Icon name="library" size={24} stroke={2.2} color="#A78BFA" /></button>
<button onClick={() => setShowSettingsModal(true)} aria-label="ParamÃ¨tres" style={{ background: "none", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: "#A78BFA" }}><Icon name="settings" size={24} stroke={2.1} color="#A78BFA" /></button>
</div>
</div>
...
{ label: "TERMINÃS", value: termines, icon: <Icon name="checkBadge" size={28} stroke={2.2} color="#34D399" />, border: "#059669", glow: "#059669" },
].map(stat => (
...
{["Tous", "En cours", "TerminÃ©s", "PDF", "Crochet", "Tricot"].map(f => (
...
{filtered.length === 0 && <div style={{ textAlign: "center", color: "#6B6A7A", padding: "40px 0", fontSize: 14 }}>Aucun projet trouvÃ©</div>}
</div>
<div style={{ position: "fixed", bottom: 28, right: "calc(50% - 200px)", zIndex: 50 }}>
...
<p style={{ color: "#6B6A7A", fontSize: 12, margin: "0 0 12px" }}>Colle ici le texte copiÃ© depuis l'export. <span style={{ color: "#F87171" }}>Attention : tes projets actuels seront remplacÃ©s.</span></p>
...
alert("â
 Projets restaurÃ©s !" + (pdfCount > 0 ? `

${pdfCount} projet(s) PDF â tu devras rÃ©importer les fichiers PDF manuellement depuis ton tÃ©lÃ©phone.` : “”));
} else {
alert(“â Texte invalide â assure-toi de coller une sauvegarde Kaleido.”);
}
} catch { alert(“â Erreur â le texte ne semble pas valide.”); }
...
<p style={{ color: "#6B6A7A", fontSize: 12, margin: "0 0 4px" }}>Ce texte contient <span style={{ color: "#A78BFA", fontWeight: 700 }}>tous tes projets, rangs et donnÃ©es</span>.</p>
<p style={{ color: "#6B6A7A", fontSize: 12, margin: "0 0 12px" }}>1. Appuie sur <strong style={{ color: "#F1F0EE" }}>Copier</strong> â 2. Ouvre <strong style={{ color: "#F1F0EE" }}>Notes</strong> â 3. Colle et sauvegarde. Pour restaurer, copie ce texte et utilise <strong style={{ color: "#F1F0EE" }}>Importer</strong> dans âï¸.</p>
...
navigator.clipboard.writeText(exportData).then(() => alert("â
 CopiÃ© dans le presse-papier !")).catch(() => alert("SÃ©lectionne le texte manuellement et copie-le."));
} else { alert("SÃ©lectionne le texte dans le champ et copie-le manuellement."); }
...
<div onClick={e => e.stopPropagation()} style={{ background: "#1A1A2E", borderRadius: "24px 24px 0 0", padding: "24px 20px 40px", width: "100%", maxWidth: 430 }}>
<div style={{ width: 36, height: 4, background: "#333", borderRadius: 2, margin: "0 auto 20px" }} />
<h2 style={{ color: "#F1F0EE", fontFamily: "'Syne', sans-serif", fontSize: 18, margin: "0 0 20px", textAlign: "center", display: "flex", alignItems: "center", justifyContent: "center", gap: 10 }}><Icon name="settings" size={18} color="#A78BFA" />ParamÃ¨tres</h2>
{/* Export â tÃ©lÃ©charge un fichier .json */}
<button onClick={async () => {
try {
...
<div style={{ color: "#F1F0EE", fontSize: 16, fontWeight: 700, fontFamily: "'DM Sans', sans-serif", marginBottom: 2 }}>Exporter mes donnÃ©es</div>
<div style={{ color: "#6B6A7A", fontSize: 12, fontFamily: "'DM Sans', sans-serif" }}>TÃ©lÃ©charge un fichier <strong style={{ color: "#A78BFA" }}>.json</strong> avec tous tes projets et PDFs</div>
</div>
</button>
{/* Import â charge depuis un fichier .json */}
...
<div style={{ textAlign: "left" }}>
<div style={{ color: "#F1F0EE", fontSize: 16, fontWeight: 700, fontFamily: "'DM Sans', sans-serif", marginBottom: 2 }}>Importer mes donnÃ©es</div>
...
setShowSettingsModal(false);
alert('â
 DonnÃ©es restaurÃ©es avec succÃ¨s !');
} catch(e) {
...
<div style={{ color: "#6B6A7A", fontSize: 13, fontFamily: "'DM Sans', sans-serif" }}>{(database.patrons || []).length} patron{(database.patrons||[]).length > 1 ? 's' : ''} dans ta bibliothÃ¨que</div>
</div>
</button>
)}
{/* Aller crÃ©er un patron */}
...
<div>
<div style={{ color: "#F1F0EE", fontSize: 16, fontWeight: 700, fontFamily: "'DM Sans', sans-serif", marginBottom: 4, display: "flex", alignItems: "center", gap: 10 }}>Aller Ã  la bibliothÃ¨que</div>
<div style={{ color: "#6B6A7A", fontSize: 13, fontFamily: "'DM Sans', sans-serif" }}>CrÃ©e ou importe un patron d'abord</div>
</div>
</button>
</div>
</div>
...
{/* SÃ©lection patron â grille de bulles */}
...
<p style={{ color: "#6B6A7A", fontSize: 12, textAlign: "center", margin: "0 0 16px", fontFamily: "'DM Sans', sans-serif" }}>Appuie sur une bulle pour crÃ©er le projet</p>
...
// âââ VUE PATRON EDITOR ââââââââââââââââââââââââââââââââââââ
const PatronEditorView = () => {
// Source : patron de bibliothÃ¨que OU projet
...
errors.push(“Partie invalide: entrÃ©e non valide.”);
continue;
}
...
if (partie.id == null) {
  ...
  errors.push(`Partie dupliquÃ©e: ${partie.id}`);
} else {
...
for (const rang of rangs) {
  ...
  if (rang.id == null) {
    ...
    errors.push(`Rang dupliquÃ©: ${rang.id}`);
  } else {
    rangIds.add(rang.id);
  }
}
...
```
  if (!next || typeof next !== "object") {
    console.warn(`[KALEIDO] ${label}: Ã©tat ignorÃ© (valeur invalide).`);
    return prev;
  }
...
return (
  ...
  <div style={{ background: "#0D0D1A", minHeight: "100vh", fontFamily: "'DM Sans', sans-serif", maxWidth: 430, margin: "0 auto" }}>
    ...
    <div style={{ background: "linear-gradient(180deg, #1A0A2E 0%, #0D0D1A 100%)", padding: "44px 20px 20px", position: "sticky", top: 0, zIndex: 10 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
        <button onClick={isPatronMode ? navigateToLibrary : navigateToHub} style={{ background: "#1E1E32", border: "none", borderRadius: 10, width: 36, height: 36, display: "flex", alignItems: "center", justifyContent: "center", color: "#A78BFA", fontSize: 16, cursor: "pointer" }}>â</button>
        {isEditingNom
      ...
      <div style={{ color: "#A78BFA", fontSize: 13, fontFamily: "monospace" }}>{patron.parties.length} parties â¢ {totalRangsPatron} rangs</div>
    </div>
    <div style={{ padding: "0 20px 16px" }}>
      <div style={{ background: "#1A1A2E", borderRadius: 16, padding: 16, border: "1px solid #ffffff0A" }}>
        ...
        <div style={{ marginBottom: 12 }}>
          ...
          <input value={patron.laine} onChange={e => updatePatronInfo("laine", e.target.value)} placeholder="Ex: Coton peignÃ©, Laine mÃ©rinos DK..."
...
// âââ RENDU CONDITIONNEL âââââââââââââââââââââââââââââââââââ
// ââ Splash Screen ââââââââââââââââââââââââââââââââââââââââââââââ
if (showSplash) {
...
const viewWrapStyle = (trans) => {
if (trans === ‘slide-in-right’) return { animation: ‘slideInRight 0.32s cubic-bezier(0.25,0.46,0.45,0.94) both’ };
if (trans === ‘slide-in’) return { animation: ‘slideInLeft 0.32s cubic-bezier(0.25,0.46,0.45,0.94) both’ };
return {};
};
...
// Toujours prendre la version fraÃ®che de database
...
{/* Photo modal pour la bibliothÃ¨que */}
...
{/* Modale Ã©dition patron PDF â dans KaleidoHub pour accÃ¨s direct Ã  database */}
{editingPdfPatron && (
<EditPdfPatronModal
