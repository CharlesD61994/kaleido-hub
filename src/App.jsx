...
const colors = [”#A78BFA”,”#F472B6”,”#34D399”,”#FCD34D”,”#22D3EE”,”#FB923C”,”#fff”,”#EC4899”];
...
const br = shape === “circle” ? “50%” : shape === “square” ? “3px” : “50% 0 50% 0”;
return (
<div key={i} style={{
...
<div className="bubble-press" style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8, padding: "10px 4px 14px", cursor: "pointer", filter: "saturate(1.02)" }}
onClick={() => onProjectClick && onProjectClick(project)}
onTouchStart={(e) => { e.currentTarget.style.transform = "scale(0.93) translateY(2px)"; e.currentTarget.style.filter = "saturate(1.02) brightness(1.12)"; }}
onTouchEnd={(e) => { e.currentTarget.style.transform = ""; e.currentTarget.style.filter = "saturate(1.02)"; }}
onTouchCancel={(e) => { e.currentTarget.style.transform = ""; e.currentTarget.style.filter = "saturate(1.02)"; }}
onMouseDown={(e) => { e.currentTarget.style.transform = "scale(0.93) translateY(2px)"; e.currentTarget.style.filter = "saturate(1.02) brightness(1.12)"; }}
onMouseUp={(e) => { e.currentTarget.style.transform = ""; e.currentTarget.style.filter = "saturate(1.02)"; }}
onMouseLeave={(e) => { e.currentTarget.style.transform = ""; e.currentTarget.style.filter = "saturate(1.02)"; }}>
...
{/* Shimmer overlay */}
<div style={{ position:"absolute", inset:0, borderRadius:"50%", background:"linear-gradient(105deg, transparent 30%, rgba(255,255,255,0.18) 50%, transparent 70%)", backgroundSize:"200% 100%", animation:"shimmerSlide 3.2s ease-in-out infinite", pointerEvents:"none", zIndex:2 }} />
</div>
...
style={{ transition: "stroke-dashoffset 0.8s cubic-bezier(0.34,1.56,0.64,1)", filter: `drop-shadow(0 0 4px ${color.light})` }} />
</svg>
...
<div style={{ background: `linear-gradient(90deg, ${currentPartieColor.bg}, ${currentPartieColor.light})`, width: `${(currentPartieRangIndex + 1) / currentPartieTotal * 100}%`, height: "100%", transition: "width 0.5s cubic-bezier(0.34,1.56,0.64,1)", position: "relative", overflow: "hidden" }}>
  <div style={{ position: "absolute", inset: 0, background: "linear-gradient(90deg, transparent 0%, rgba(255,255,255,0.3) 50%, transparent 100%)", animation: "shimmerSlide 2.2s ease-in-out infinite" }} />
</div>
</div>
</div>
</div>
...
@keyframes bounceFab {
  0%, 100% { transform: scale(1) rotate(0deg); box-shadow: 0 4px 20px #7C3AED88; }
  30% { transform: scale(1.18) rotate(8deg); box-shadow: 0 8px 32px #7C3AEDcc; }
  60% { transform: scale(0.94) rotate(-4deg); box-shadow: 0 2px 12px #7C3AED66; }
}
...
.fab-bounce:active { animation: bounceFab 0.45s cubic-bezier(0.36,0.07,0.19,0.97) both; }
.bubble-press { transition: transform 160ms cubic-bezier(0.34,1.56,0.64,1), filter 180ms ease; }
.bubble-press:active { transform: scale(0.93) translateY(2px) !important; filter: brightness(1.15) saturate(1.3) !important; }
`}</style>
...
return (
<div onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 300, background: "rgba(0,0,0,0.85)", display: "flex", alignItems: "flex-end", animation: "fadeIn 0.22s ease both", justifyContent: "center", fontFamily: "'DM Sans', sans-serif" }}>
<div onClick={e => e.stopPropagation()} style={{ background: "#1A1A2E", borderRadius: "24px 24px 0 0", padding: "24px 20px 40px", width: "100%", maxWidth: 430, maxHeight: "90vh", overflowY: "auto", border: "1px solid #0891B244", animation: "slideUp 0.32s cubic-bezier(0.34,1.56,0.64,1) both" }}>
        ...
        <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", justifyContent: "flex-start", marginTop: -4 }}>
          {currentPartie ? (<>
            ...
            <div style={{ background: "rgba(255,255,255,0.08)", borderRadius: 6, height: 4, overflow: "hidden", marginTop: 2, marginBottom: 16 }}>
              <div style={{ background: `linear-gradient(90deg, ${color.bg}, ${color.light})`, width: `${totalPartieCourante > 0 ? Math.round(rangDansPartie / totalPartieCourante * 100) : 0}%`, height: "100%", transition: "width 0.5s cubic-bezier(0.34,1.56,0.64,1)", position: "relative", overflow: "hidden" }}>
                <div style={{ position: "absolute", inset: 0, background: "linear-gradient(90deg, transparent 0%, rgba(255,255,255,0.35) 50%, transparent 100%)", animation: "shimmerSlide 2s ease-in-out infinite" }} />
              </div>
            </div>
          </>) : total > 0 ? (<>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <div style={{ background: "rgba(255,255,255,0.08)", borderRadius: 6, height: 4, overflow: "hidden", marginTop: 2, marginBottom: 16 }}>
              <div style={{ background: `linear-gradient(90deg, ${color.bg}, ${color.light})`, width: `${pct}%`, height: "100%", transition: "width 0.5s cubic-bezier(0.34,1.56,0.64,1)", position: "relative", overflow: "hidden" }}>
                <div style={{ position: "absolute", inset: 0, background: "linear-gradient(90deg, transparent 0%, rgba(255,255,255,0.35) 50%, transparent 100%)", animation: "shimmerSlide 2s ease-in-out infinite" }} />
              </div>
            </div>
          </>) : <div style={{ marginBottom: 16 }} />}
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, transform: "translateX(-16px)" }}>
            <button onClick={decrementRang}
              onTouchStart={e => { e.currentTarget.style.transform = "scale(0.88)"; }}
              onTouchEnd={e => { e.currentTarget.style.transform = "scale(1)"; }}
              onMouseDown={e => { e.currentTarget.style.transform = "scale(0.88)"; }}
              onMouseUp={e => { e.currentTarget.style.transform = "scale(1)"; }}
              onMouseLeave={e => { e.currentTarget.style.transform = "scale(1)"; }}
              style={{ width: 42, height: 42, borderRadius: "50%", background: `${color.bg}33`, border: `1.5px solid ${color.light}44`, color: color.light, fontSize: 22, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", transition: "transform 120ms cubic-bezier(0.34,1.56,0.64,1)" }}>−</button>
            ...
            <button onClick={incrementRang} className="fab-bounce"
              onTouchStart={e => { e.currentTarget.style.transform = "scale(0.88)"; }}
              onTouchEnd={e => { e.currentTarget.style.transform = "scale(1)"; }}
              onMouseDown={e => { e.currentTarget.style.transform = "scale(0.88)"; }}
              onMouseUp={e => { e.currentTarget.style.transform = "scale(1)"; }}
              onMouseLeave={e => { e.currentTarget.style.transform = "scale(1)"; }}
              style={{ width: 42, height: 42, borderRadius: "50%", background: `linear-gradient(135deg, ${color.bg}, ${color.light})`, border: "none", color: "#fff", fontSize: 22, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: `0 3px 10px ${color.bg}55`, transition: "transform 120ms cubic-bezier(0.34,1.56,0.64,1)" }}>+</button>
          </div>
        </div>
      </div>
      {/* Menu swipe */}
...
function PdfViewerView({ project, onNavigateHub, onSaveProgress }) {
  ...
  return (
    <div style={{ background: "#0D0D1A", height: "100vh", fontFamily: "'DM Sans', sans-serif", maxWidth: 430, margin: "0 auto", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      {showFinModal && (
        <>
          <Confetti active={showFinModal} />
          <div style={{ position: "fixed", inset: 0, zIndex: 200, background: "rgba(0,0,0,0.85)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
            <div style={{ background: "#1A1A2E", borderRadius: 24, padding: 28, width: "100%", maxWidth: 360, textAlign: "center", border: `1px solid ${color.light}33`, animation: "modalPop 0.42s cubic-bezier(0.34,1.56,0.64,1) both" }}>
              {/* Etoiles animées */}
              <div style={{ position: "relative", fontSize: 52, marginBottom: 12 }}>
                <span style={{ display: "inline-block", animation: "starBurst 0.6s ease 0.1s both" }}>★</span>
                <span style={{ display: "inline-block", animation: "starBurst 0.6s ease 0.25s both", marginLeft: 6, fontSize: 36 }}>✨</span>
                <span style={{ display: "inline-block", animation: "starBurst 0.6s ease 0.4s both", marginLeft: 6 }}>★</span>
              </div>
              <div style={{ color: color.light, fontSize: 13, fontFamily: "monospace", textTransform: "uppercase", letterSpacing: 1, marginBottom: 8, animation: "slideUp 0.4s ease 0.15s both" }}>Projet terminé !</div>
              <h2 style={{ color: "#F1F0EE", fontSize: 20, fontFamily: "'Syne', sans-serif", margin: "0 0 8px", animation: "slideUp 0.4s ease 0.2s both" }}>{project?.name}</h2>
              <p style={{ color: "#6B6A7A", fontSize: 14, margin: "0 0 24px", animation: "slideUp 0.4s ease 0.25s both" }}>Félicitations, tu as complété tous les rangs !</p>
              <button onClick={() => setShowFinModal(false)}
                className="fab-bounce"
                style={{ width: "100%", padding: "14px", borderRadius: 14, border: "none", background: `linear-gradient(135deg, ${color.bg}, ${color.light})`, color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "'DM Sans', sans-serif", animation: "slideUp 0.4s ease 0.32s both" }}>
                Super !
              </button>
            </div>
          </div>
    ...
    <div style={{ background: "#0D0D1A", minHeight: "100vh", fontFamily: "'DM Sans', sans-serif" }}>
      <style>{`
        ...
        * { -webkit-tap-highlight-color: transparent; }
        ...
        @keyframes bounceFab { 0%, 100% { transform: scale(1) rotate(0deg); } 30% { transform: scale(1.18) rotate(8deg); } 60% { transform: scale(0.94) rotate(-4deg); } }
        @keyframes slideUp { from { transform: translateY(40px); opacity: 0; } to { transform: translateY(0); opacity: 1; } }
        @keyframes modalPop { 0% { transform: scale(0.8); opacity: 0; } 70% { transform: scale(1.04); opacity: 1; } 100% { transform: scale(1); opacity: 1; } }
        @keyframes infoBob { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(-1px); } }
        .fab-bounce:active { animation: bounceFab 0.45s cubic-bezier(0.36,0.07,0.19,0.97) both; }
        .bubble-press { transition: transform 200ms cubic-bezier(0.34,1.56,0.64,1), filter 180ms ease; }
        .bubble-press:active { transform: scale(0.93) translateY(2px) !important; filter: brightness(1.15) saturate(1.3) !important; }
      `}</style>
      {/* Header */}
      ...
      <div style={{ position: "fixed", bottom: 28, right: "calc(50% - 200px)", zIndex: 50 }}>
        <button onClick={() => setShowNewMenu(true)} className="fab-bounce" style={{ width: 56, height: 56, borderRadius: "50%", background: "linear-gradient(135deg, #7C3AED, #EC4899)", border: "none", cursor: "pointer", fontSize: 28, color: "#fff", boxShadow: "0 4px 20px #7C3AED88", display: "flex", alignItems: "center", justifyContent: "center" }}>+</button>
      </div>
      {/* Menu contextuel */}
      ...
      {/* Menu nouveau patron */}
      {showNewMenu && (
        <div onClick={() => setShowNewMenu(false)} style={{ position: "fixed", inset: 0, zIndex: 200, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "flex-end", animation: "fadeIn 0.22s ease both", justifyContent: "center" }}>
...
<div style={{ background: "#0D0D1A", minHeight: "100vh", fontFamily: "'DM Sans', sans-serif" }}>
<style>{`@import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600;700&family=Syne:wght@700;800&display=swap'); @keyframes fadeIn { from { opacity:0; transform:translateY(10px); } to { opacity:1; transform:translateY(0); } } @keyframes slideInRight { from { opacity:0; transform:translateX(28px); } to { opacity:1; transform:translateX(0); } } @keyframes slideInLeft { from { opacity:0; transform:translateX(-28px); } to { opacity:1; transform:translateX(0); } } @keyframes shimmerSlide { 0% { transform: translateX(-100%); } 100% { transform: translateX(100%); } } @keyframes bounceFab { 0%, 100% { transform: scale(1) rotate(0deg); } 30% { transform: scale(1.18) rotate(8deg); } 60% { transform: scale(0.94) rotate(-4deg); } } @keyframes starBurst { 0% { transform: scale(0) rotate(0deg); opacity: 1; } 60% { transform: scale(1.4) rotate(180deg); opacity: 1; } 100% { transform: scale(0) rotate(360deg); opacity: 0; } } @keyframes slideUp { from { transform: translateY(40px); opacity: 0; } to { transform: translateY(0); opacity: 1; } } @keyframes modalPop { 0% { transform: scale(0.8); opacity: 0; } 70% { transform: scale(1.04); opacity: 1; } 100% { transform: scale(1); opacity: 1; } } @keyframes confettiFall { 0% { transform: translateY(-20px) rotate(0deg); opacity: 1; } 100% { transform: translateY(120px) rotate(720deg); opacity: 0; } } @keyframes infoBob { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(-1px); } } ::-webkit-scrollbar { width: 0; } * { -webkit-tap-highlight-color: transparent; } input, textarea, select { font-size: 16px !important; } .fab-bounce:active { animation: bounceFab 0.45s cubic-bezier(0.36,0.07,0.19,0.97) both; } .bubble-press { transition: transform 200ms cubic-bezier(0.34,1.56,0.64,1), filter 180ms ease; } .bubble-press:active { transform: scale(0.93) translateY(2px) !important; filter: brightness(1.15) saturate(1.3) !important; }`}</style>
...
<div key={project.id} style={{ animation: `fadeIn 0.3s ease ${idx * 0.045}s both` }}>
...
<div style={{ position: "fixed", bottom: 28, right: "calc(50% - 200px)", zIndex: 50 }}>
<button onClick={handleNewProject} className="fab-bounce" style={{ width: 56, height: 56, borderRadius: "50%", background: "linear-gradient(135deg, #7C3AED, #EC4899)", border: "none", cursor: "pointer", fontSize: 28, color: "#fff", boxShadow: "0 4px 20px #7C3AED88", display: "flex", alignItems: "center", justifyContent: "center" }}>+</button>
</div>
...
{showSettingsModal && (
<div onClick={() => setShowSettingsModal(false)} style={{ position: "fixed", inset: 0, zIndex: 200, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "flex-end", animation: "fadeIn 0.22s ease both", justifyContent: "center" }}>
<div onClick={e => e.stopPropagation()} style={{ background: "#1A1A2E", borderRadius: "24px 24px 0 0", padding: "24px 20px 40px", width: "100%", maxWidth: 430, animation: "slideUp 0.32s cubic-bezier(0.34,1.56,0.64,1) both" }}>
<div style={{ width: 36, height: 4, background: "#333", borderRadius: 2, margin: "0 auto 20px" }} />
...
if (trans === ‘slide-in-right’) return { animation: ‘slideInRight 0.38s cubic-bezier(0.34,1.2,0.64,1) both’ };
if (trans === ‘slide-in’) return { animation: ‘slideInLeft 0.38s cubic-bezier(0.34,1.2,0.64,1) both’ };
return {};
};
